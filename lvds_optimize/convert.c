...
const long nYLen = (iHeight * iWidth);  // 32

const int nHfWidth = (iWidth>>1); // 4
...
//rgb[2] =(int)(yData[i] + 1.732446 * (uData[j] - 128)); // b·ÖÁ¿Öµ 
rgb[2] = (int)(yData[i] + 1.370705 * (vData[j] - 128));
  

rgb[1] = (int)(yData[i] - 0.698001 * (uData[j] - 128) - 0.703125 * (vData[j] - 128)); // g·ÖÁ¿Öµ  

//rgb[0] = (int)(yData[i] + 1.370705 * (vData[j] - 128)); // r·ÖÁ¿Öµ  
rgb[0] = (int)(yData[i] + 1.732446 * (uData[j] - 128));;
  

j = nYLen - iWidth - m + x;   //24 
...
inline char ClipColorValue(int x)
{
return x < 0 ? 0 : (x > 255 ? 255 : x);
}


inline void YUV420_RGB32_4Pixel(char *pRGB, char *pY, char *pU, char  *pV, int Width)
{
int nRGB4Y = 0;
...
{
char  *pY = pYUV,   pU = pY + Width * Height,   pV = pU + Width * Height/4;
...
bool convertYUV420ToRGB32(const char *yuv,int width,int height,char **pRgb,int *realLen)
{
	...
	unsigned char * yData = (unsigned char *)yuv;   //!y·ÖÁ¿ÆðÊ¼µØÖ·
	unsigned char * uData = &yData[len];			//!u·ÖÁ¿ÆðÊ¼µØÖ·
	unsigned char * vData = &uData[len>>2];			//!v·ÖÁ¿ÆðÊ¼µØÖ·
	const int nRgbLen = width *height *4;			//!RGB32 Êý¾Ý³¤¶È
	char rgb[nRgbLen];				//!·ÖÅäRGB32ÄÚ´æ
	...
	for(int i = 0 ; i < height ; i++ )
	{
		for(int j = 0;j < width; j++ )
		{
			...
			//!YUV420¸ñÊ½ Ã¿4¸öY ¹²ÓÃÒ»¸öUV¡£
			yIdx = i * width + j;				//!YÆðÊ¼Ë÷Òý		* * * * * * * *
			uIdx = (i/2) * (width/2) + (j/2);	//!UÆðÊ¼Ë÷Òý		 @   @   @   @	
			vIdx = uIdx;						//!VÆðÊ¼Ë÷Òý		* * * * * * * * 
			

			bgr[0] = (int)(yData[yIdx] + 1.779 * (uData[vIdx] - 128));		//b·ÖÁ¿

			bgr[1] = (int)(yData[yIdx] - 0.3455 * (uData[uIdx] - 128) - 0.7169 * (vData[vIdx] - 128)); //g·ÖÁ¿

			bgr[2] = (int)(yData[yIdx] + 1.4075 * (vData[uIdx] - 128));		//!r·ÖÁ¿


			for(int k = 0;k < 3;k++)
			{
				idx = (i * width + j) * 4 + k ;		//!RGBÊý¾ÝË÷Òý	RGB32Êý¾Ý´æ´¢¸ñÊ½ [b][g][r][alph] 4¸ö×Ö½ÚÎªÒ»¸öRGB32Êý¾Ý
