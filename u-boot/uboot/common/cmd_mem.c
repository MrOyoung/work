...
* do_mem_hwmtest
 */
int do_mem_hwmtest (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
...
int do_mem_dataQloopw (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	ulong	addr, length,i;
	int	size;
#ifdef CONFIG_SYS_SUPPORT_64BIT_DATA
...
U_BOOT_CMD(
	dataQloopw,    3,    1,    do_mem_dataQloopw,
	"dataQloopw   - infinite write loop on address \n",
	"[.b, .w, .l, .q] address  data_to_write\n"
	"    - loop on a  addresses\n"
);
