...
#define ENET_GPIO16_PAD_CTRL  (PAD_CTL_PUS_100K_UP |			\
	PAD_CTL_SPEED_HIGH | PAD_CTL_DSE_80ohm | PAD_CTL_HYS | PAD_CTL_SRE_FAST)
...
iomux_v3_cfg_t const enet_pads[] = {
	...
	MX6_PAD_GPIO_16__ENET_REF_CLK		| MUX_PAD_CTRL(ENET_GPIO16_PAD_CTRL),
...
{
	struct iomuxc *iomux = (struct iomuxc *)IOMUXC_BASE_ADDR;/*0x020e0000*/
	u32 reg;

	reg = readl(&iomux->gpr[1]);
	reg  |=(0x01<<21);	
	writel(reg, &iomux->gpr[1]);/*enet_ref_clk select internal*/
...
static void setup_lvds_iomux(void)
{
	...
	imx_pwm_config(pwm, 5000000, 5000000);
	imx_pwm_enable(pwm);
...
int logo_show(unsigned char *pic)
{
	unsigned char * pData;

	pData = (unsigned char *)CONFIG_FB_BASE;
	
	memcpy(pData, pic, DISPLAY_WIDTH * DISPLAY_HEIGHT * (DISPLAY_BPP / 8));
...
#if 0
	pData = (unsigned char *)(CONFIG_FB_BASE + DISPLAY_WIDTH * DISPLAY_HEIGHT * (DISPLAY_BPP / 8));

	memset(pData, 0x33, DISPLAY_WIDTH * DISPLAY_HEIGHT * (DISPLAY_BPP / 8));
...
int logo_buf_init(unsigned char *pic, int index)
{
	int i;
	int j;
	...
	if(index == 0){	
		...
		// Red
		for(i = 0; i < (DISPLAY_WIDTH * (DISPLAY_BPP / 8) / 3) ; i += (DISPLAY_BPP / 8)){
			for(j = 0; j < DISPLAY_HEIGHT; j++){
				pic[i + j * DISPLAY_WIDTH * (DISPLAY_BPP / 8) + 0] = 0x00;
				pic[i + j * DISPLAY_WIDTH * (DISPLAY_BPP / 8) + 1] = 0x00;
				pic[i + j * DISPLAY_WIDTH * (DISPLAY_BPP / 8) + 2] = 0xFF;
				pic[i + j * DISPLAY_WIDTH * (DISPLAY_BPP / 8) + 3] = 0xFF;
			}
		}
		...
		for(; i < (DISPLAY_WIDTH * (DISPLAY_BPP / 8) / 3) * 2 ; i += (DISPLAY_BPP / 8)){
			for(j = 0; j < DISPLAY_HEIGHT; j++){
				pic[i + j * DISPLAY_WIDTH * (DISPLAY_BPP / 8) + 0] = 0x00;
				pic[i + j * DISPLAY_WIDTH * (DISPLAY_BPP / 8) + 1] = 0xFF;
				pic[i + j * DISPLAY_WIDTH * (DISPLAY_BPP / 8) + 2] = 0x00;
				pic[i + j * DISPLAY_WIDTH * (DISPLAY_BPP / 8) + 3] = 0x00;
			}
		}
	

		// Blue
		for(; i < (DISPLAY_WIDTH * (DISPLAY_BPP / 8) ) ; i += (DISPLAY_BPP / 8)){
			for(j = 0; j < DISPLAY_HEIGHT; j++){
				pic[i + j * DISPLAY_WIDTH * (DISPLAY_BPP / 8) + 0] = 0xFF;
				pic[i + j * DISPLAY_WIDTH * (DISPLAY_BPP / 8) + 1] = 0x00;
				pic[i + j * DISPLAY_WIDTH * (DISPLAY_BPP / 8) + 2] = 0x00;
				pic[i + j * DISPLAY_WIDTH * (DISPLAY_BPP / 8) + 3] = 0x00;
			}
...
#else
static int logo_blank(void)
{
	unsigned char * pData;

	pData = (unsigned char *)CONFIG_FB_BASE;

	memset(pData, 0x0, DISPLAY_WIDTH * DISPLAY_HEIGHT * (DISPLAY_BPP / 8));
...
int board_late_init(void)
{
	...
	unsigned char *pic_buf1=NULL;
	unsigned char *pic_buf2=NULL;

	index = 0;

	pic_buf1 = malloc(DISPLAY_WIDTH * DISPLAY_HEIGHT * (DISPLAY_BPP / 8));
	pic_buf2 = malloc(DISPLAY_WIDTH * DISPLAY_HEIGHT * (DISPLAY_BPP / 8));

	logo_buf_init(pic_buf1, 1);
	logo_buf_init(pic_buf2, 0);

	logo_show(pic_buf1);
	printf("uboot show logo\n"); 
#else
	logo_blank();
#endif	
