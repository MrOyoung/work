...
//	ipu_channel_params.eba1 = ipu_channel_params.eba0;
	ipu_channel_params.eba1 = (DISPLAY_EBA0 + DISPLAY_WIDTH * DISPLAY_HEIGHT * (DISPLAY_BPP / 8)) / 8;
...
void dump_ipu_reg(void)
{
	unsigned int  i = 0;
	unsigned int count = 0;

	printf("\ndump_ipu_reg:\n");
	for(i= 0x2600000; i < 0x2668020; i += 4){
		
		if(count%10 == 0){
			printf("\n    %x : ", i);
			printf("%x ", __raw_readl(i));
		}else{
...
void dump_clk_reg(void)
{
	unsigned int  i = 0;
	unsigned int count = 0;

	printf("\ndump_clk_reg:\n");
	for(i= 0x20C4000; i < 0x20C4000 + 0x4280; i += 4){

		if(i >= (0x20C4000 + 0x88)  && i < (0x20C4000 + 0x4000)){
			continue;
		}

		if(i > (0x20C4000 + 0x4260)  && i < (0x20C4000 + 0x4280)){
			continue;
		}
